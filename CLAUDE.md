...
| `supabase/schema.sql` | 15 tables, 6 views. Reasoning lives in the comments |
...
npm run verify            # 21 token-drift + 29 contrast + 27 schema + 9 deploy
...
`shots` enforces what replaced the old bans: no external requests, Inter only,
radii on the 8px scale, and no state carried by colour alone.
...
- **Station Manager only in v1.** Everyone else exists as a name that can hold
  a role and sign out kit, not as an account.
...
- **Delete is allowed on `events` and `tasks` only.** Kit bookings, ledger
...
`funding_windows`), playbook and handover export, safeguarding log. There is **no server, no database
deployment, and no Next.js app** — the prototype is a single self-contained
file persisting to `localStorage`. Export gives back a corrected `year.json`.

**Deploying.** `vercel.json` builds to `dist/` and ships a tight CSP;
`npm run verify:deploy` proves the artifact survives it in real Chrome.
